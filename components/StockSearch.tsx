    ...
    <div className="space-y-6">
      <form onSubmit={handleSearch} className="relative group">
        <input
          ...
          className="w-full bg-black border border-white/10 rounded-xl px-4 py-5 pl-12 text-sm font-mono text-skin-text focus:outline-none focus:border-skin-accent/50 focus:ring-1 focus:ring-skin-accent/50 transition-all uppercase tracking-widest placeholder:opacity-20 shadow-inner"
        />
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-skin-muted group-focus-within:text-skin-accent transition-colors" />
        <button
          disabled={loading || !ticker}
          className="absolute right-3 top-1/2 -translate-y-1/2 bg-skin-accent hover:bg-skin-accent/90 disabled:opacity-50 text-black px-5 py-2.5 rounded-lg text-xs font-bold tracking-tighter transition-all flex items-center gap-2 min-w-[110px] justify-center shadow-[0_0_15px_rgba(var(--color-accent),0.2)]"
        >
          {loading ? (
            <>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-4 gap-2">
            ...
            <button 
                ...
                className={`text-[10px] font-mono border border-white/5 px-2 py-2 rounded-lg bg-white/5 transition-all hover:bg-skin-accent/10 hover:border-skin-accent/30 hover:text-skin-accent shadow-sm ${ticker === t ? 'text-skin-accent border-skin-accent/30 bg-skin-accent/5 ring-1 ring-skin-accent/20' : 'text-skin-muted'}`}
            >
                {t}
            </button>
            ))}
